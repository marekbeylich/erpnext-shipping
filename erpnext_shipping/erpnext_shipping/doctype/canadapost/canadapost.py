...
#   UPDATING A FILE TO POST CANADA. DO NOT PUSH TO GITHUB UNTIL DONE
#   THERE IS A COMMENT WHERE THE WORK WAS LEFT OFF, ANYTHING ABOVE CAN BE ASSUMED TO BE COMPLETED


class CanadaPost(Document): pass

class CanadaPostUtils():
	def __init__(self):
		self.api_password = get_decrypted_password('CanadaPost', 'CanadaPost', 'api_password', raise_exception=False)
		self.api_user, self.enabled = frappe.db.get_value('CanadaPost', 'CanadaPost', ['api_user', 'enabled'])

		if not self.enabled:
			link = frappe.utils.get_link_to_form('CanadaPost', 'CanadaPost', frappe.bold('CanadaPost Settings'))
			frappe.throw(_('Please enable CanadaPost Integration in {0}'.format(link)), title=_('Mandatory'))
