...
class FedEx(Document): pass

class FedExUtils():
	def __init__(self):
		self.secret_key = get_decrypted_password('FedEx', 'FedEx', 'secret_key', raise_exception=False)
		self.api_key, self.enabled = frappe.db.get_value('FedEx', 'FedEx', ['api_key', 'enabled'])

		if not self.enabled:
			link = frappe.utils.get_link_to_form('FedEx', 'FedEx', frappe.bold('FedEx Settings'))
			frappe.throw(_('Please enable FedEx Integration in {0}'.format(link)), title=_('Mandatory'))
		
	def get_authtoken(self):
		
		#Get Oauth2.0 token from FedEx for API requests

		oauthurl = "https://apis-sandbox.fedex.com/oauth/token"
		payload='grant_type=client_credentials&client_id={0}client_secret={1}'.format(self.secret_key, self.api_key)
		...
		try:
			...
			if 'access_token' in response:
				...
				frappe.throw(_('An Error occurred while fetching FedEx API Authtoken: {0}')
					.format(response_data['message']))
		except Exception:
			show_error_alert("fetching FedEx API Authtoken")
	...
	#		'email': contact.email
	#	}
	#     self.get_pickup_delivery_info(pickup_address, pickup_contact),



	def get_available_services(self, pickup_address,
		delivery_address, shipment_parcel, description_of_content,
		value_of_goods):
		# Retrieve rates at FedEx from specification stated.
		if not self.enabled or not self.api_key or not self.secret_key:
			return []
		...
		auth = 'Bearer {0}'.format(self.get_authtoken)
		...
		payload = self.generate_min_payload(
	...
	def generate_payload(self, pickup_address, pickup_contact, delivery_address, delivery_contact,
		...
		payload = {
			'shipper': self.get_pickup_delivery_info(pickup_address, pickup_contact),
			'recipients': self.get_pickup_delivery_info(delivery_address, delivery_contact),
			'shipmentDetails': {
	...
	def generate_min_payload(self, pickup_address, delivery_address, description_of_content, value_of_goods, parcel_list):
		weight= parcel_list[0]['weight']
		payload = {
			'requestedShipment': {
				'shipper': self.get_pickup_delivery_info(pickup_address),
				'recipients': self.get_pickup_delivery_info(delivery_address),
